...
func main() {
	...
	studentScores := make(map[string][]int) // Create map[string][]int
	...
	gradebook := make(map[string]map[string]int) // Create map[string]map[string]int
	...
	mathGrades, mathExists := gradebook["Math"] // Check if "Math" exists in gradebook
	if mathExists {
		aliceMath, aliceExists := mathGrades["Alice"] // Check if "Alice" exists in Math grades
		if aliceExists {
	...
	people := make(map[string]Person) // Create map[string]Person
